...
namespace social.Controllers
{
        ...
        public async Task<ActionResult> Index(string Id)
        {
            ...
            if (Id == null)
            {
                ...
                if (authUser.Photo == null)
                    ...
                    ViewBag.AvatarURL = Url.Content("~/Images/") + "/" + authUser.Photo;
                ViewBag.UserName = authUser.Email;
                IEnumerable<Photo> photos = connection.Query<Photo>("select * from Photos where SenderId=@userId ORDER BY PublishedTime DESC", new { userId = authUser.UserId });
                ViewBag.photoCount = photos.Count();
                foreach (Photo ph in photos)
                {
                    User postUser = authUser;
                    Post ps = new Post();
                    ps.Id = ph.Id;
            ...
            {
                User user = connection.Query<User>("select * from Users where UserId = @userId", new { userId = Id }).SingleOrDefault();
                if (user.Photo == null)
                    user.Photo = "avatar3.png";
                ...
                if (user.Photo == null)
                    ...
                    ViewBag.AvatarURL = Url.Content("~/Images/") + "/" + authUser.Photo;
                ViewBag.UserName = authUser.Email;
                IEnumerable<Photo> photos = connection.Query<Photo>("select * from Photos where SenderId=@userId ORDER BY PublishedTime DESC", new { userId = user.UserId });
                ViewBag.photoCount = photos.Count();
                foreach (Photo ph in photos)
                {
                    if (ph.isPrivate)
                    ...
                    ps.PublishedTime = ph.PublishedTime.ToString();
                    ps.SenderId = ph.SenderId;
                    ps.SenderName = postUser.UserName;
                    ps.SenderPhoto = postUser.Photo;
                    if (postUser.Photo == null)
                        ps.SenderPhoto = "avatar3.png";
                    posts.Add(ps);
    ...
    [AllowAnonymous]
        ...
        {
            User usr = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            EditUserViewModel mod = new EditUserViewModel { SurName=usr.SurName, Email= usr.Email, BirthDay = usr.BirthDay,  Gender = usr.Gender, PhotoURL = usr.Photo  };
            return View(mod);
        }
        ...
        public async Task<ActionResult> Edit(EditUserViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Photo != null)
                    usr.Photo = SavePhotoAndGetName(model.Photo, usr.UserName);
