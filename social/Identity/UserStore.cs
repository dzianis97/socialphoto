...
namespace social.Identity
{
    public class UserStore : IUserStore<User>, IUserLoginStore<User>, IUserPasswordStore<User>, IUserSecurityStampStore<User>
    {
        ...
        public virtual Task CreateAsync(User user)
        {
            ...
            return Task.Factory.StartNew(() => {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                    connection.Execute("insert into Users(UserId, UserName, PasswordHash, SecurityStamp, Photo, Email, Gender, SurName, BirthDay, isAdmin) values(@userId, @userName, @passwordHash, @securityStamp, @photo, @email, @gender, @surName, @birthDay, @isAdmin)", user);
            });
        }
        ...
        public virtual Task<User> FindByIdAsync(string userId)
        {
            ...
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                    return connection.Query<User>("select * from Users where UserId = @userId", new { userId = userId }).SingleOrDefault();
            });
        }
        ...
        public virtual Task UpdateAsync(User user)
        {
            ...
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                    connection.Execute("update Users set UserName = @userName, PasswordHash = @passwordHash, SecurityStamp = @securityStamp, Photo = @photo, Email = @email, Gender = @gender, SurName = @surName, BirthDay = @birthDay, isAdmin = @isAdmin where UserId = @userId", user);
            });
        }
        #endregion
